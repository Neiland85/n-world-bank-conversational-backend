...
async function loadSecrets() {
    if (process.env.NODE_ENV === 'production') {
      ...
      console.log(
        ...
        } mode. Secrets not loaded from Key Vault.`
        );
  }
}
/**
 ...
 try {
  if (!process.env.MONGO_URI) {
   throw new Error('MONGO_URI is not defined in environment variables.');
    ...
    await mongoose.connect(process.env.MONGO_URI); // Connection without options useNewUrlParser or useUnifiedTopology
...
Archivo .env para entornos desarrollo o production:
...
# Nombre del Key Vault de Azure
KEY_VAULT_NAME=mi-keyvault-real

# URI MongoDB ( se cargará dinamicamente si no hay vault configurado, )
MONGO_URI=mongodb://admin:securepassword@localhost:27017/auth-service-db?authSource=admin
